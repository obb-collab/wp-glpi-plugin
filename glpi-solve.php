...
function gexe_glpi_ticket_resolve() {
    ...
    if ($ticket_id <= 0) {
        error_log('[resolve] NO_PERMISSION ticket=' . $ticket_id . ' wp=' . $wp_uid . ' glpi=0');
        wp_send_json(['error' => 'NO_PERMISSION'], 404);
    }

    if (!is_user_logged_in()) {
        error_log('[resolve] NONCE_EXPIRED ticket=' . $ticket_id . ' wp=' . $wp_uid . ' glpi=0');
        wp_send_json(['error' => 'NONCE_EXPIRED'], 401);
    }
    $author_glpi = gexe_get_current_glpi_user_id($wp_uid);
    if ($author_glpi <= 0) {
        error_log('[resolve] NO_GLPI_USER ticket=' . $ticket_id . ' wp=' . $wp_uid . ' glpi=0');
    ...
    if (!$exists) {
        error_log('[resolve] NO_PERMISSION ticket=' . $ticket_id . ' wp=' . $wp_uid . ' glpi=' . $author_glpi);
        wp_send_json(['error' => 'NO_PERMISSION'], 404);
    }
    ...
    if (!$glpi_db->query($sql)) {
        ...
        gexe_log_action(sprintf('[resolve.sql] ticket=%d author=%d result=fail code=SQL_OP_FAILED msg="%s"', $ticket_id, $author_glpi, $err));
        error_log('[resolve] SQL_OP_FAILED status_update_failed ticket=' . $ticket_id . ' wp=' . $wp_uid . ' glpi=' . $author_glpi . ' sql=' . $err);
    ...
    if (!$f['ok']) {
        $glpi_db->query('ROLLBACK');
        if (($f['code'] ?? '') === 'SQL_ERROR') {
            gexe_log_action(sprintf('[resolve.sql] ticket=%d author=%d result=fail code=SQL_OP_FAILED msg="%s"', $ticket_id, $author_glpi, $f['message'] ?? ''));
            error_log('[resolve] SQL_OP_FAILED followup_insert_failed ticket=' . $ticket_id . ' wp=' . $wp_uid . ' glpi=' . $author_glpi . ' sql=' . ($f['message'] ?? ''));
            wp_send_json(['error' => 'SQL_OP_FAILED'], 500);
        }
